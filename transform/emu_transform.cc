...
struct EmuTransformPass : public Pass {
    ...
    void execute(vector<string> args, Design* design) override
    {
        ...
        Pass::call(design, {"emu_port_transform", "-tracebackend", ckpt_path.substr(0, pos) + "/TraceBackend.v"});
        ...
        std::ifstream in_file(ckpt_path.substr(0, pos) + "/TraceBackend.v"); 
        if(in_file.is_open()){
            Pass::call(design, {"read_verilog", "-noautowire", ckpt_path.substr(0, pos) + "/TraceBackend.v"});
        }
        Pass::call(design, "hierarchy");
        ...
        if (!raw_plat)
            Pass::call(design, "emu_integrate_system");
