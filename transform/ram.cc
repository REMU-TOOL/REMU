...
struct MemoryDffWorker
{
    ...
    //    the merge.
    bool handle_rd_port(Mem &mem, QuickConeSat &qcsat, int idx, bool dry_run)
    {
    ...
    bool handle_rd_port_addr(Mem &mem, int idx, bool dry_run)
    {
        auto &port = mem.rd_ports[idx];
    ...
    void run(std::vector<Mem> &memories)
    {
        for (auto &mem : memories) {
            // dry-run
            bool ok = true;
            log("Dry-running on memory %s ...\n", mem.memid.c_str());
            QuickConeSat qcsat(modwalker);
            for (int i = 0; i < GetSize(mem.rd_ports); i++) {
                if (!mem.rd_ports[i].clk_enable)
                    ok &= (handle_rd_port(mem, qcsat, i, true) || handle_rd_port_addr(mem, i, true));
            }
            if (!ok) {
                log("Not all read ports can be convert to synchronous one.\n");
                continue;
            }
            // actually convert to sync read ports
            log("Converting memory %s ...\n", mem.memid.c_str());
            for (int i = 0; i < GetSize(mem.rd_ports); i++) {
                if (!mem.rd_ports[i].clk_enable)
