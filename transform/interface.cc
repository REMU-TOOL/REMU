...
AXI::AXI4Lite AXIHelper::axi4lite_from_prefix(Module *module, const std::string &prefix)
{
    AXI::AXI4Lite axi;
...
AXI::AXI4 AXIHelper::axi4_from_prefix(Module *module, const std::string &prefix)
{
    AXI::AXI4 axi;
    FROM_PREFIX(aw, valid);
    FROM_PREFIX(aw, ready);
...
void InterfaceWorker::promote_axi_intfs(Module *module)
{
    ...
    for (auto portid : module->ports) {
        ...
        std::string type = wire->get_string_attribute(Attr::ExternIntfType);

        if (type != "axi4")
            continue;

        AXIIntfInfo info;
        info.port_name = info.orig_name = wire->get_string_attribute(Attr::ExternIntf);
        info.axi = AXIHelper::axi4_from_prefix(module, info.orig_name);
        info.pages = -1;

        info.axi.check();

        if (wire->has_attribute(Attr::ExternIntfAddrPages))
            info.pages = wire->attributes.at(Attr::ExternIntfAddrPages).as_int();

        log("Identified AXI4 interface %s with addr_pages = %d\n", info.orig_name.c_str(), info.pages);
    ...
    for (auto &edge : node.outEdges()) {
        ...
        for (auto &info : all_axi_intfs.at(child.name)) {
            ...
            newinfo.port_name = "EMU_AXI_" + path_prefix(newinfo.path) + info.port_name;
            ...
            for (
                ...
                Wire *wire = module->addWire(new_it->name);
