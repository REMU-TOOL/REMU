...
class TB:
    def __init__(self, dut):
        ...
        self.axi_ram = AxiRam(AxiBus.from_prefix(dut, 'target_dma'), dut.target_clk, dut.target_rst, size=0x10000)
...
async def run_test(dut):
    ...
    async def axilite_read_write_worker_func(addr):
        for step in range(0, 100):
            size = random.randint(0, 3)
            test_data = range(0,256)
            dut._log.info("STEP %d: address=0x%x, axsize=%d " %
                          (step, addr, size))
            await ClockCycles(dut.host_clk, random.randint(1, 20))
            await tb.axilite_master.write(addr, test_data,size=size)
            await ClockCycles(dut.host_clk, random.randint(1, 20))
            data = await tb.axilite_master.read(addr,size=size)
            # dut.log.info("test_data: " + repr(test_data))
            # dut.log.info("data.data: " + repr(data.data))
