...
launch:
	...
	rm -rf /tmp/ckpt
	python3 -m host $(LOADMEM) $(DESIGN_BUILD_DIR)/scanchain.yml $(HOST) $(PORT) /tmp/ckpt
...
reconstruct: $(DESIGN_SIM_BIN)
	...
	python3 -m host --dump $(STARTCYCLE) --dumpfile $(DESIGN_BUILD_DIR)/$(STARTCYCLE).hex $(DESIGN_BUILD_DIR)/scanchain.yml $(HOST) $(PORT) /tmp/ckpt
...
$(DESIGN_OUTPUT_V): $(VSRC)
	...
	$(YOSYS) -m transform -p "emu_transform -top $(VTOP) -sc_yaml $(DESIGN_BUILD_DIR)/scanchain.yml" -o $@ $^
